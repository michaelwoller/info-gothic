...
I started doing a lot of these as R Shiny apps, only to learn that shinyapps.io caps free accounts to only 5 apps. So, only the first five lessons are interactable. But that's ok, the static lessons are just as good :\^)

Data for static exercises if you wish to follow along can be found in [this Google Drive](https://drive.google.com/file/d/1DGBIRSJygyH9FIedPYdnYPDBGkt1Mhnv/view?usp=sharing) link. This also includes heavily annotated R code that more or less goes over what is discussed in these lectures, but if you want to view it in R comments instead, feel free.

One note, in the R code lessons in this zip, I talk a lot about the "PROCESS Macro". This was something the professor wanted to provide as an option to students (since PROCESS is well equipped to handle mediation/moderation analysis that Psychology graduate students might do in the future). PROCESS does not come as a installable package in R you can use via functions in R. Instead, you have to manually go to the website and download it yourself. Legally, I cannot distribute the macro or else I would. The [website is here](https://haskayne.ucalgary.ca/CCRAM/resource-hub). Click on "Download PROCESS v5.0", which is the version that these tutorials were built upon. You need to navigate to the PROCESS R file and load it in R. The file is a very long script of functions. You must run *all* of this script to be able to use PROCESS functions. In all honestly, I would not recommend learning this unless you know you will want to run mediation analysis models sometime in your future. This was mainly a quirk that was included in these scripts at the time that I left in and just wanted to explain. For this purpose, the lessons I have linked below do not discuss PROCESS at all.

The lessons build on each other sequentially. So you will not understand as well later lessons if you do not work through previous lessons.
...
[Open Tutorial (Static Page)](regression-lessons/simple-linear-regression.html)
...
[Open Tutorial (Static Page)](regression-lessons/multiple-linear-regression.html)
...
[Open Tutorial (Static Page)](regression-lessons/inference-and-multiple-correlations.html)
...
[Open Tutorial (Static Page)](regression-lessons/diagnostics.html)
...
[Open Tutorial](regression-lessons/variable-importance.html)
...
[Open Tutorial](regression-lessons/prediction.html)
...
[Open Tutorial](regression-lessons/multicategorical-predictors.html)
...
[Open Tutorial](regression-lessons/nonlinear-regression.html)
...
[Open Tutorial](regression-lessons/moderation.html)
...
[Open Tutorial](regression-lessons/logistic-regression.html)
