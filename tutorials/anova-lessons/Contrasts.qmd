...
$$ \Psi_2 = \frac{-1}{3}\bar{X}_{drug~therapy}+\frac{-1}{3}\bar{X}_{biofeedback}+\frac{-1}{3}\bar{X}_{diet}} + (1)\bar{X}_{combined} $$ $$ \Psi_2=-1\bar{X}_{drug~therapy}+-1\bar{X}_{biofeedback}+-1\bar{X}_{diet} +(3)\bar{X}_{combined} $$ If you recall, this will not change your significance test or your F statistic. All that will change is your confidence interval. So in SPSS, if you have to do this, you *need to remember* to rescale the confidence interval after by dividing it by the number you scaled the coefficients by (in this case, take the confidence interval and divide both sides by 3).
...
2.  **The special matrix must always be a square matrix**, with the number of columns and rows equal to $a$, the number of groups. Within the MANOVA function, you can test $a - 1$ contrasts per special matrix. With our data of 4 groups, we can only test 3 contrasts at a time, and we *must* test three contrasts at a time. You have no choice in this. If you were interested in more than $a-1$ contrasts (e.g., in this case I wanted to test 4 contrasts), then you have to run multiple MANOVA procedures just with different contrasts in the special matrix.
